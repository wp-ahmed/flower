...
function wc_get_gallery_image_html( $attachment_id, $main_image = false ) {
	...
	$image             = wp_get_attachment_image(
		...
		apply_filters(
			...
			array(
				...
				'class'                   => esc_attr( $main_image ? 'product-big-img' : '' ),
			),
			$attachment_id,
			$image_size,
			$main_image
	...
	return '<div  data-thumb="' . esc_url( $thumbnail_src[0] ) . '" data-thumb-alt="' . esc_attr( $alt_text ) . '" class="product-pic-zoom">' . $image . '</div>';
}
