 ...
 * @version  2.5.0
 */
/* Modify class of select at wc_dropdown_variation_attribute_options function */
/* modify product image wc_get_gallery_image_html */
...
function wc_get_gallery_image_html( $attachment_id, $main_image = false ) {
	...
	$image             = wp_get_attachment_image(
		...
		apply_filters(
			...
			array(
				...
				'class'                   => esc_attr( $main_image ? 'wp-post-image' : 'product-big-img' ),
			),
			$attachment_id,
			$image_size,
			$main_image
	...
	return '<div data-thumb="' . esc_url( $thumbnail_src[0] ) . '" data-thumb-alt="' . esc_attr( $alt_text ) . '" class="product-pic-zoom woocommerce-product-gallery__image"><a href="' . esc_url( $full_src[0] ) . '">' . $image . '</a></div>';
}
...
if ( ! function_exists( 'wc_dropdown_variation_attribute_options' ) ) {
	...
	function wc_dropdown_variation_attribute_options( $args = array() ) {
		...
		$html  = '<select id="' . esc_attr( $id ) . '" class=form-control form-control-lg"' . esc_attr( $class ) . '" name="' . esc_attr( $name ) . '" data-attribute_name="attribute_' . esc_attr( sanitize_title( $attribute ) ) . '" data-show_option_none="' . ( $show_option_none ? 'yes' : 'no' ) . '">';
